...
public class PasswordResetController {
	...
	public RedirectView remoteAuthenticate(String token, HttpSession session, HttpServletRequest request) {
		SecurityContextHolder.getContext().setAuthentication(cbcAuthenticationProvider.authenticate(new UsernamePasswordAuthenticationToken(cbcService.getAppToken(), token)));
		return new RedirectView(request.getContextPath() + "/reset/passwordreset");
	}
}
